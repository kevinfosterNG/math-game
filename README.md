...
2. In Google Auth Platform, create a web OAuth client. Set the production site origin (and local development origin if needed) as authorized JavaScript origins. Set the authorized redirect URI to the exact Supabase callback URL shown on the Supabase Google provider page. In Supabase Authentication, enable Google and enter the client ID and secret there. Configure the Site URL and redirect allowlist for the production site and local development URL. Enable only the sign-in providers you intend to offer; disable anonymous Auth sign-ins. The browser's `redirectTo` is the current site origin.

3. Copy `.env.example` to `.env.local` and set only the project URL and publishable key. Existing `SUPABASE_URL` and `SUPABASE_PUBLISHABLE_KEY` values are mapped by Vite to these public names. Add those two public values as build secrets in GitHub Actions. Keep the Google client secret in Supabase, and never put a database password or service-role key in Vite variables.
